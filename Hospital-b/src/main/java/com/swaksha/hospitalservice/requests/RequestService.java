...
//    private RequestController.ConsentObj1 consent1Of(RequestController.ConsentObj consentObj){
//        return new RequestController.ConsentObj1(consentObj.doctorSSID(), consentObj.hiuSSID(), consentObj.patientSSID(),
//                consentObj.hipSSID(), LocalDate.parse(consentObj.dataAccessStartDate()), LocalDate.parse(consentObj.dataAccessEndDate()),
//                LocalDate.parse(consentObj.requestInitiatedDate()), LocalDate.parse(consentObj.consentApprovedDate()),
//                LocalDate.parse(consentObj.consentEndDate()), consentObj.consentID(), consentObj.selfConsent(), consentObj.isApproved());
//    }
    ...
    private Consent consentOf(RequestController.ConsentObj consentObj){
        return new Consent(consentObj.consentID(), consentObj.consentEndDate(), consentObj.isApproved(),
                consentObj.selfConsent(), consentObj.doctorSSID(), consentObj.hiuSSID(), consentObj.patientSSID(),
                consentObj.hipSSID(), consentObj.dataAccessStartDate(), consentObj.dataAccessEndDate(),
                consentObj.requestInitiatedDate(), consentObj.consentApprovedDate());
    }
}
